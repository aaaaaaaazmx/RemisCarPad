    ...
    public abstract class BaseFloatView extends FrameLayout implements View.OnTouchListener {
        ...
        private void initView() {
            ...
            post(() -> {
                ...
                if (getAdsorbType() == ADSORB_VERTICAL) {
                    adsorbTopAndBottom(null);
                } else if (getAdsorbType() == ADSORB_HORIZONTAL) {
                    adsorbLeftAndRight(null);
                }
                if (getAdsorbTime() > 0) {
        ...
        private void adsorbTopAndBottom(MotionEvent event) {
            if (event == null || isOriginalFromTop()) {
                // 上半屏或首次加载
                double centerY = mViewHeight / 2 + (event != null ? Math.abs(event.getRawY() - mFirstY) : 0);
        ...
        private void adsorbLeftAndRight(MotionEvent event) {
            if (event == null || isOriginalFromLeft()) {
                // 左半屏或首次加载
                double centerX = mViewWidth / 2 + (event != null ? Math.abs(event.getRawX() - mFirstX) : 0);
